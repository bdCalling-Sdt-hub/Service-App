...
class HelpController extends GetxController{
  ...
  RxList<HelpModel> providerHelpModel=<HelpModel>[].obs;

  ScrollController scrollController=ScrollController();

  Future providerhelpFirsLoad()async{
    firstLoading(true);
    var response=await ApiClient.getData('${ApiConstants.providerHelpEndPoint}?page=$page&limit=5');
    if(response.statusCode==200){
      providerHelpModel.value= List<HelpModel>.from(response.body['data']['attributes'].map((x) => HelpModel.fromJson(x)));
  ...
  loadMore()async{
    if(firstLoading !=true &&loadMoreLoading ==false && totalPage !=currentPage){
      ...
      if(response.statusCode==200){
        ...
        providerHelpModel.value.addAll(result);
        providerHelpModel.refresh();
        rxRequestStatus(Status.completed);
